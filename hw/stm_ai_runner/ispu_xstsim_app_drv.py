...
###################################################################################
"""
ISPU simulator xstsim driver (legacy)
"""
import os
...
class IspuXstsimAppDriver(AppDriver):
    """
    ...
    def __init__(self, parent):
        ...
        super(IspuXstsimAppDriver, self).__init__(parent)
    ...
    def _clean(self):
        ...
        """
        files = [
            'sim-0000.prof.c0.callgrind',
            'sim-0000.prof.c0.cycles.tmp',
            'sim-0000.prof.c0.gprof.cycles',
            'sim-0000.prof.c0.info',
            'sim-0000.stats.raw'
        ]
        for file in files:
    ...
    def connect(self, desc=None, **kwargs):
        ...
        self.gprof_dump_file = kwargs['context'].get_option('validate.ispu_profile')
        ...
        target_series = kwargs['context'].get_device_info().get_generic_device_name()
        if target_series != "imu22":
            raise AiRunnerError(f"{target_series} not supported by xstsim simulator")
    ...
    def write_sim_config(self, profiling_file_name, sample):
        ...
        with open(profiling_file_name, 'w') as fp:
            ...
            dump_stats             "raw"
            accuracy               "functional"
            var target_exec       "{app_name}"
            [c0]
            type    "ReISC-L"
            target_exec            _.target_exec
            target_args           ["{sample}"]
            [c0.profiler]
            metrics                ["cycles"]
            dump_at                {cfg_dump}
    ...
    def short_desc(self):
        ...
        """
        return 'ISPU XSTSIM app' + ' (' + self._app + ')'
